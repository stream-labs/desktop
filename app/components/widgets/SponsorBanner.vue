<template>
  <widget-editor :slots="[{ value: 'layout', label: $t('Ad Layout') }]" :navItems="navItems">
    <validated-form slot="layout" @input="save()" v-if="loaded">
      <v-form-group :title="$t('Placement')">
        <list-input
          v-model="wData.settings.placement_options"
          :metadata="{ options: placementOptions }"
        />
      </v-form-group>
      <v-form-group
        v-if="wData.settings.placement_options === 'double'"
    ...
    >
      <v-form-group :title="`${$t('Placement')} ${position} ${$t('Images')}`">
        <div
          v-for="image in wData.settings[`placement_${position}_images`]"
          :key="image.href"
          class="media-container"
        >
          <media-gallery-input
            v-model="image.href"
            :metadata="{ fileName: fileNameFromHref(image.href) }"
          />
          <button class="close-button" @click="removeImage(image.href, position)">
            <i class="icon-close" />
          </button>
        ...
        <button class="button button--default" @click="addImage(position)">
          {{ $t('Add Image') }}
        </button>
      </v-form-group>
    </validated-form>
    ...
    <validated-form slot="visual-properties" @input="save()" v-if="loaded">
      <v-form-group
        :title="$t('Widget Hide Duration')"
        :metadata="{ tooltip: hideDurationTooltip }"
      >
        <timer-input v-model="wData.settings.hide_duration_in_seconds" :metadata="{}" />
      </v-form-group>
      <v-form-group
        :title="$t('Widget Show Duration')"
        :metadata="{ tooltip: showDurationTooltip }"
      >
      ...
      <v-form-group :title="$t('Banner Height')">
        <slider-input
          v-model="wData.settings.banner_height"
          :metadata="{ max: 720, interval: 5 }"
        />
      </v-form-group>
      ...
      <v-form-group :title="$t('Background Color')">
        <bool-input
          v-model="wData.settings.background_color_option"
          :metadata="{ title: $t('Transparent') }"
        />
        <color-input
          v-if="!wData.settings.background_color_option"
          v-model="wData.settings.background_container_color"
        />
      </v-form-group>
    </validated-form>
  </widget-editor>
