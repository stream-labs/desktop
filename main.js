...
////////////////////////////////////////////////////////////////////////////////
const { VCRedist } = require('./tools/vsredist.js');
{
  function statusCallback(data) {
    ...
    if (data.status.checking) {
      ...
      console.log("Downloading file... " + (data.percent * 100).toFixed(2) + "% (" + data.downloadBytes + " of " + data.totalBytes + " bytes downloaded)")
  ...
  async function waitForUpdate(cb) {
    let res = await (new Promise((resolve, reject) => {
      resolve(vcredist.update(statusCallback));
    }));
    cb(res);
  }

  let vcredist = new VCRedist();
  if (vcredist.check()) {
    waitForUpdate((res) => {
      if(res == true) {
        app.relaunch();
        ...
        // Show an error or something. This is usually an indicator that something went wrong and support should be contacted.
      }
    });
  } else {
    slobsApplication();
