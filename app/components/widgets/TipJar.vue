<template>
  <widget-editor :slots="[{ value: 'jar', label: $t('Jar Image') }]" :navItems="navItems">
    ...
    <validated-form slot="manage-jar-properties" @input="save()" v-if="loaded">
      ...
      <v-form-group v-if="wData.settings.types.twitch_bits" :title="$t('Minimum Bits')">
        <number-input
          v-model="wData.settings.types.twitch_bits.minimum_amount"
          :metadata="{ required: true, min: 1 }"
        />
      </v-form-group>
      <v-form-group :title="$t('Minimum Tips')">
        <number-input
          v-model="wData.settings.types.tips.minimum_amount"
          :metadata="{ required: true, min: 1 }"
        />
      </v-form-group>
      <v-form-group
        ...
        :metadata="{ description: backgroundColorDescription }"
      />
    </validated-form>
    ...
    <validated-form slot="font-properties" @input="save()" v-if="loaded">
      ...
      <v-form-group
        :title="$t('Text Color')"
        type="color"
        v-model="wData.settings.text.color"
