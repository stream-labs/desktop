...
test('The default scene', async t => {
  const app = t.context.app;
  await focusMain(t);
  t.true(await app.client.isExisting('[data-test="Scene"]'));
...
test('Adding and removing a scene', async t => {
  ...
  t.true(await app.client.isExisting(`[data-test="${sceneName}"]`));

  await selectScene(t, sceneName);
  await checkDefaultSources(t);
  await clickRemoveScene(t);

  t.false(await app.client.isExisting(`[data-test="${sceneName}"]`));
});
...
test('Rename scene', async t => {
  ...
  t.true(await app.client.isExisting(`[data-test="${newSceneName}"]`));
});